...
ls -d tests/* | grep -v -e 'volar' -e 'puppet' -e 'verible' -e 'svls' | xargs -I% basename % | xargs -I% -P 2 bash -c 'set -euo pipefail; export TEST=%; ((make setup | tee .tests-output/${TEST}.log) || true); make test | tee .tests-output/${TEST}.log'

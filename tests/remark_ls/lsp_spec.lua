...
describe("remark_ls", function()
  helpers.setup_server("remark_ls", {
    root_dir = util.root_pattern("root-marker"),
  })

  it("starts", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | find fixtures/example-project-1/my-project/main.md")
    ...
    assert.equal("remark_ls", buf_clients[1].name)
  ...
  it("works with single file support", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-2/main.md")
    vim.api.nvim_command("set ft=md")
    helpers.wait_for_ready_lsp()

...
describe("lelwel_ls", function()
  helpers.setup_server("lelwel_ls", {
    root_dir = util.root_pattern("root-marker"),
  })

  it("starts", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-1/src/main.llw")
    vim.api.nvim_command("set ft=llw")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("lelwel_ls", buf_clients[1].name)

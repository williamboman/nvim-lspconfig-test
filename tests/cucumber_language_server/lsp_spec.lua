...
describe("cucumber_language_server", function()
  local workspace_dir = helpers.resolve_workspace_dir("example-project-1")

  helpers.setup_server("cucumber_language_server", {
    root_dir = function()
  ...
  it("starts", function()
    ...
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-1/index.feature")
    vim.api.nvim_command("set ft=cucumber")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("cucumber_language_server", buf_clients[1].name)

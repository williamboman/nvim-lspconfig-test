...
describe("dhall_lsp_server", function()
  helpers.setup_server("dhall_lsp_server", {
    root_dir = util.root_pattern("root-marker"),
  })

  it("starts", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | find fixtures/example-project-1/main.dhall")
    vim.api.nvim_command("set ft=dhall")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("dhall_lsp_server", buf_clients[1].name)
  ...
  it("works with single file support", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-2/main.dhall")
    vim.api.nvim_command("set ft=dhall")
    helpers.wait_for_ready_lsp()

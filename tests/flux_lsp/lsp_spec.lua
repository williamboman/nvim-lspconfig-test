...
describe("flux_lsp", function()
  helpers.setup_server("flux_lsp", {
    root_dir = util.root_pattern("root-marker"),
  })

  it("starts", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | find fixtures/example-project-1/my-project/main.flux")
    vim.api.nvim_command("set ft=flux")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("flux_lsp", buf_clients[1].name)
  ...
  it("works with single file support", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-2/main.flux")
    vim.api.nvim_command("set ft=flux")
    helpers.wait_for_ready_lsp()

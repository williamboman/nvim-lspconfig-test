...
describe("foam_ls", function()
  helpers.setup_server("foam_ls", {})

  it("starts", function()
    vim.api.nvim_command("bufdo bwipeout!")
    vim.api.nvim_command("new | only | find fixtures/example-project-1/my-project/main.foam")
    vim.api.nvim_command("set ft=foam")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("foam_ls", buf_clients[1].name)

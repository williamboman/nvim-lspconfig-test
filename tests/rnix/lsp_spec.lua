...
describe("rnix", function()
  ...
  it("starts", function()
    ...
    vim.api.nvim_command("new | only | silent edit fixtures/example-project-1/app/main.nix")
    vim.api.nvim_command("set ft=nix")
    helpers.wait_for_ready_lsp()

    local buf_clients = vim.tbl_values(vim.lsp.buf_get_clients(0))

    assert.equal(1, #buf_clients)
    assert.equal("rnix", buf_clients[1].name)
